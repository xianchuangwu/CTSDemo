...
import com.chad.ctsdemo.Utils.InputSurface;
import com.chad.ctsdemo.Utils.OutputSurface;
...
public class MainActivity extends AppCompatActivity {
    ...
    public void MuxAudio(View view) {
        extractDecodeEditEncodeMuxAudio();

    }

    public void MuxVideoAudio(View view) {
        extractDecodeEditEncodeMuxVideoAudio();
    ...
    public void DownloadPlayTogether(View view) {
     ...
     */
    private void extractDecodeEditEncodeMux() throws Throwable {
        //TODO
     ...
     * 解码编码后得到的只有视频文件
     */
    public void extractDecodeEditEncodeMuxVideo() {
//        setSize(176, 144);//QCIF
//        setSize(320, 240);//QVGA
        setSize(1280, 720);//720p
        setSource(R.raw.meipai_20170120105449_ori);
        setCopyVideo();
        setOutputFile();
     ...
     * 解码编码后得到的只有音频文件
     */
    public void extractDecodeEditEncodeMuxAudio() {
        setSize(1280, 720);//720p
        setSource(R.raw.meipai_20170120105449_ori);
        setCopyAudio();
        setOutputFile();
     ...
     * 解码编码后得到的是音视频文件
     */
    public void extractDecodeEditEncodeMuxVideoAudio() {
        setSize(1280, 720);//720p
        setSource(R.raw.meipai_20170120105449_ori);
        setCopyAudio();
        setCopyVideo();
     ...
     */
    private MediaExtractor createExtractor() throws Throwable {
    ...
    private MediaCodec createVideoEncoder(MediaCodecInfo codecInfo, MediaFormat format,
                                          AtomicReference<Surface> surfaceReference) throws Throwable {
     ...
     */
    private MediaCodec createVideoDecoder(MediaFormat inputFormat, Surface surface) throws Throwable {
     ...
     * @param format    of the stream to be produced
     */
    private MediaCodec createAudioEncoder(MediaCodecInfo codecInfo, MediaFormat format) throws Throwable {
        MediaCodec encoder = MediaCodec.createByCodecName(codecInfo.getName());
        encoder.configure(format, null, null, MediaCodec.CONFIGURE_FLAG_ENCODE);
        encoder.start();
        return encoder;
     ...
     * @param inputFormat the format of the stream to decode
     */
    private MediaCodec createAudioDecoder(MediaFormat inputFormat) throws Throwable {
        MediaCodec decoder = MediaCodec.createDecoderByType(getMimeTypeFor(inputFormat));
    ...
    private void doExtractDecodeEditEncodeMux(
        ...
        while ((mCopyVideo && !videoEncoderDone) || (mCopyAudio && !audioEncoderDone)) {
            ...
            while (mCopyVideo && !videoExtractorDone
                ...
                //TODO filter
                //bytebuffer --> bitmap
                byte[] imageBytes = new byte[decoderInputBuffer.remaining()];
                decoderInputBuffer.get(imageBytes);
                Bitmap bitmap = BitmapFactory.decodeByteArray(imageBytes, 0, imageBytes.length);

                //gpuimage process bitmap
                ...
                ByteBuffer newByteBuffer = ByteBuffer.wrap(byteArray);

                int size = videoExtractor.readSampleData(newByteBuffer, 0);
